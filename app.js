...
function renderTabelle() {
  ...
  const rows = alleTermine.map(function(t) {
    ...
    const _btnBase = 'display:inline-flex;align-items:center;justify-content:center;width:30px;height:28px;border-radius:7px;border:none;cursor:pointer;padding:0;';
    const aktionButtons = istAlternativ
      ? '<button onclick="meldeVerschiebung(\'' + t.id + '\', \'' + t.status + '\')" title="' + (t.status === 'Verschiebung nötig' ? 'Alternativtermin läuft' : 'Alternativtermin') + '" style="' + _btnBase + 'background:#D4620A;color:#fff;font-size:15px;font-weight:700">↔</button>'
      : '<a href="' + abfrageLink + '" target="_blank" title="Link öffnen" aria-label="Link öffnen" style="' + _btnBase + 'background:#D4A017;color:#1a1000;text-decoration:none"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#1a1000" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg></a>' +
        '<button onclick="kopierenWA(this, \'' + waText.replace(/'/g, "\\'").replace(/\n/g, '\\n') + '\')" title="WhatsApp-Text kopieren" aria-label="WhatsApp" style="' + _btnBase + 'background:#25D366"><svg width="15" height="15" viewBox="0 0 24 24" fill="#fff"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M11.914 0C5.34 0 0 5.34 0 11.914c0 2.11.549 4.094 1.508 5.818L0 24l6.459-1.474a11.882 11.882 0 005.455 1.32c6.573 0 11.914-5.34 11.914-11.914C23.828 5.34 18.487 0 11.914 0zm0 21.828a9.914 9.914 0 01-5.032-1.369l-.361-.214-3.734.852.87-3.638-.235-.374A9.865 9.865 0 012 11.914C2 6.443 6.443 2 11.914 2c5.472 0 9.914 4.443 9.914 9.914 0 5.472-4.442 9.914-9.914 9.914z"/></svg></button>' +
        '<button onclick="meldeVerschiebung(\'' + t.id + '\', \'' + t.status + '\')" title="Alternativtermin" aria-label="Alternativtermin" style="' + _btnBase + 'background:#D4620A;color:#fff;font-size:15px;font-weight:700">↔</button>' +
        '<button onclick="zeigeAufstellung(\'' + t.id + '\')" title="Aufstellung festlegen" aria-label="Aufstellung" style="' + _btnBase + 'background:#1D9E75"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg></button>';
    ...
    return '<tr style="' + rowStyle + '">' +
      ...
      '<td style="font-weight:700">' + t.gegner +
        (alleAufstellungen.some(a => a.spieltermin_id === t.id)
          ? '<span title="Aufstellung gesetzt" style="display:inline-flex;align-items:center;gap:4px;margin-left:8px;padding:2px 8px;border-radius:8px;background:rgba(29,158,117,0.18);color:#4FD4A8;font-size:10px;font-weight:700;vertical-align:middle;text-transform:uppercase;letter-spacing:0.05em">✓ Aufstellung</span>'
          : '') +
      '</td>' +
      '<td>' + (txt
